...
class NexusGeoJSON {
  ...
  render_() {
    ...
    for (const el of this.elements_) {
      NexusUtils.removeEl(el);
    }
    this.elements_ = [];

    if (this.geojson_.type === 'FeatureCollection') {
      for (const feature of this.geojson_.features) {
        const el = this.renderFeature_(feature);
        this.container_.appendChild(el);
        this.elements_.push(el);
      }
    } else if (this.geojson_.type === 'Feature') {
      const el = this.renderFeature_(this.geojson_);
      this.container_.appendChild(el);
      this.elements_.push(el);
    }
  }
